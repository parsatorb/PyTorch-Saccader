#Copy pasted from https://github.com/pytorch/examples/blob/master/imagenet/main.py
#with slight modifications:
#1. Using CartesianDataset
...
parser.add_argument('--interpolation', default=Image.BOX, type=int,
                    help='Downsampling type (enums of PIL.Image)')
...
def main_worker(gpu, ngpus_per_node, args, Dataset, centers_path, num_focal):
    ...
    if args.pretrained:
        ...
        if args.arch in custom_models:
            model = custom_models[args.arch]()
        else:
...
def validate_final(val_loader, model, criterion, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        val_loader1 = val_loader
        val_loader2 = copy.deepcopy(val_loader)  
        val_loader3 = copy.deepcopy(val_loader)
        
        val_loader1.dataset.final_loader = 2
        val_loader2.dataset.final_loader = 1
        val_loader3.dataset.final_loader = 0
        
        val_loader2 = iter(val_loader2)
        val_loader3 = iter(val_loader3)
        
        for i, (images1, target) in enumerate(val_loader1):
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True)
            target = target.cuda(args.gpu, non_blocking=True)
        
            images2, _ = next(val_loader2)
            images3, _ = next(val_loader3)
            ...
            output = torch.mean(torch.stack([output1, output2, output3]), dim=0)
            loss = criterion(output, target)
